...
def get_threads_menu(driver, url):
    try:
        ...
        filtered_lines = [l.strip() for l in lines if l.strip()]
        formatted_text = "<br>".join(filtered_lines[:15]) if filtered_lines else "메뉴 내용 없음"
        return f'<div style="background:#f9f9f9; border:1px solid #ddd; padding:10px; border-radius:8px; font-size:13px; text-align:left;">{formatted_text}</div>'
    except Exception as e:
...
for item in cafeteria_list:
    ...
    if item["type"] == "ojeong":
        ...
        html_content = f'<img src="{src}" style="display:block; margin:0 auto; max-width:100%; max-height:350px; border-radius:6px;">' if src else "<div>이미지 없음</div>"
    elif item["type"] == "kakao_posts":
        img_src = get_kakao_posts_image(driver, item["url"])
        html_content = f'<img src="{img_src}" style="display:block; margin:0 auto; max-width:100%; max-height:350px; border-radius:6px;">' if img_src else '<div>이미지 없음</div>'
